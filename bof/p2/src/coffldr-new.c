...
{
	if (symPtr->StorageClass == IMAGE_SYM_CLASS_EXTERNAL ||
		symPtr->StorageClass == IMAGE_SYM_CLASS_STATIC ||
		symPtr->StorageClass == IMAGE_SYM_CLASS_LABEL
		)
		return TRUE;
				...
				for (uint32_t j = 0; j < secHdr->NumberOfRelocations; j++)
				{
					...
					if (symHdr[relPtr->SymbolTableIndex].sn.ln.AllZero != 0) //symbol name length < 8
					{
						...
						if (relPtr->Type == IMAGE_REL_AMD64_ADDR64)
						{
							...
							if (symHdr[relPtr->SymbolTableIndex].SectionNumber > IMAGE_SYM_UNDEFINED)
							{
								symValue = symHdr[relPtr->SymbolTableIndex].Value;

								if (needAddSymValue(&symHdr[relPtr->SymbolTableIndex]))
									*tarRel = (uint64_t)secMapPtr[symHdr[relPtr->SymbolTableIndex].SectionNumber - 1] + symValue;
								else
									printf("[-] unknown StorageClass: %d\n", symHdr[relPtr->SymbolTableIndex].StorageClass);
							}
							else printf("[-] unknown symbol type\n");
						}
						else if (relPtr->Type == IMAGE_REL_AMD64_ADDR32NB || relPtr->Type == IMAGE_REL_AMD64_REL32)
						{
							...
							if (symHdr[relPtr->SymbolTableIndex].SectionNumber > IMAGE_SYM_UNDEFINED)
							{
								...
								if (needAddSymValue(&symHdr[relPtr->SymbolTableIndex]))
									*tarRel = secMapPtr[symHdr[relPtr->SymbolTableIndex].SectionNumber - 1] + symValue - ((char*)tarRel + sizeof(uint32_t));
								else
									printf("[-] unknown StorageClass: %d\n", symHdr[relPtr->SymbolTableIndex].StorageClass);
							}
						...
						else if (relPtr->Type == IMAGE_REL_AMD64_REL32_1)
						{
							...
							if (symHdr[relPtr->SymbolTableIndex].SectionNumber > IMAGE_SYM_UNDEFINED)
							{
								...
								if (needAddSymValue(&symHdr[relPtr->SymbolTableIndex]))
									*tarRel = secMapPtr[symHdr[relPtr->SymbolTableIndex].SectionNumber - 1] + symValue - ((char*)tarRel + sizeof(uint32_t) + 1);
								else
									printf("[-] unknown StorageClass: %d\n", symHdr[relPtr->SymbolTableIndex].StorageClass);
							}
						...
						else if (relPtr->Type == IMAGE_REL_AMD64_REL32_2)
						{
							...
							if (symHdr[relPtr->SymbolTableIndex].SectionNumber > IMAGE_SYM_UNDEFINED)
							{
								...
								if (needAddSymValue(&symHdr[relPtr->SymbolTableIndex]))
									*tarRel = secMapPtr[symHdr[relPtr->SymbolTableIndex].SectionNumber - 1] + symValue - ((char*)tarRel + sizeof(uint32_t) + 2);
								else
									printf("[-] unknown StorageClass: %d\n", symHdr[relPtr->SymbolTableIndex].StorageClass);
							}
						...
						else if (relPtr->Type == IMAGE_REL_AMD64_REL32_3)
						{
							...
							if (symHdr[relPtr->SymbolTableIndex].SectionNumber > IMAGE_SYM_UNDEFINED)
							{
								...
								if (needAddSymValue(&symHdr[relPtr->SymbolTableIndex]))
									*tarRel = secMapPtr[symHdr[relPtr->SymbolTableIndex].SectionNumber - 1] + symValue - ((char*)tarRel + sizeof(uint32_t) + 3);
								else
									printf("[-] unknown StorageClass: %d\n", symHdr[relPtr->SymbolTableIndex].StorageClass);
							}
						...
						else if (relPtr->Type == IMAGE_REL_AMD64_REL32_4)
						{
							...
							if (symHdr[relPtr->SymbolTableIndex].SectionNumber > IMAGE_SYM_UNDEFINED)
							{
								...
								if (needAddSymValue(&symHdr[relPtr->SymbolTableIndex]))
									*tarRel = secMapPtr[symHdr[relPtr->SymbolTableIndex].SectionNumber - 1] + symValue - ((char*)tarRel + sizeof(uint32_t) + 4);
								else
									printf("[-] unknown StorageClass: %d\n", symHdr[relPtr->SymbolTableIndex].StorageClass);
							}
						...
						else if (relPtr->Type == IMAGE_REL_AMD64_REL32_5)
						{
							...
							if (symHdr[relPtr->SymbolTableIndex].SectionNumber > IMAGE_SYM_UNDEFINED)
							{
								...
								if (needAddSymValue(&symHdr[relPtr->SymbolTableIndex]))
									*tarRel = secMapPtr[symHdr[relPtr->SymbolTableIndex].SectionNumber - 1] + symValue - ((char*)tarRel + sizeof(uint32_t) + 5);
								else
									printf("[-] unknown StorageClass: %d\n", symHdr[relPtr->SymbolTableIndex].StorageClass);
							}
						...
						if (relPtr->Type == IMAGE_REL_I386_DIR32)
						{
							...
							if (symHdr[relPtr->SymbolTableIndex].SectionNumber > IMAGE_SYM_UNDEFINED)
							{
								...
								if (needAddSymValue(&symHdr[relPtr->SymbolTableIndex]))
									*tarRel = (uint32_t)secMapPtr[symHdr[relPtr->SymbolTableIndex].SectionNumber - 1] + symValue;
								else
									printf("[-] unknown StorageClass: %d\n", symHdr[relPtr->SymbolTableIndex].StorageClass);
							}
							else printf("[-] unknown symbol type\n");
					...
					else
					{
						...
						else if (relPtr->Type == IMAGE_REL_AMD64_REL32 || relPtr->Type == IMAGE_REL_AMD64_ADDR32NB)
						{
							uint32_t* tarRel = (uint32_t*)(secMapPtr[i] + relPtr->VirtualAddress);
							uint32_t symValue = 0;
